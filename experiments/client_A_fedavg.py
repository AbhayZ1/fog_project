"""
RUN 2: FedAvg Client A — WITH Differential Privacy (same as original)
Identical to original client_A.py — DP stays enabled.
FedAvg on server side only removes the proximal term.
"""
import os
import torch
...
print(f"\n{'='*70}")
print(f"🏥 [FedAvg] CLIENT A (Hospital A) - DP Enabled")
print(f"{'='*70}")
...
def load_data():
    data_dir = os.path.join(os.path.dirname(__file__), "..", "demo", "Hospital_A", "local_storage", "data")
...
def train(net, trainloader, optimizer, epochs):
    ...
    for epoch in range(epochs):
        for i, (inputs, labels) in enumerate(trainloader):
            ...
            optimizer.step()
            if i % 10 == 0:
                print(f"   [FedAvg Client A] Epoch {epoch+1}/{epochs}, Batch {i}/{len(trainloader)}, Loss: {loss.item():.4f}")

    epsilon = privacy_engine.get_epsilon(delta=TARGET_DELTA)
    print(f"   [FedAvg Client A] Privacy spent: ε={epsilon:.2f}")
...
class PneumoniaClient(fl.client.NumPyClient):
    ...
    def fit(self, parameters, config):
        ...
        print(f"\n🔄 [FedAvg Client A] Training (10 epochs)...")
    ...
    def evaluate(self, parameters, config):
        ...
        print(f"\n[FedAvg CLIENT A EVAL] ε={privacy_info['epsilon']:.2f} | "

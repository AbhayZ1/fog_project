"""
RUN 2: FedAvg Baseline Server
Uses FedAvg instead of FedProx (no proximal term).
DP is still enabled on clients.
Saves metrics to fl_metrics_fedavg.json.
"""
import flwr as fl
import torch
...
MODELS_DIR = "model_registry_fedavg"
...
METRICS_FILE = "fl_metrics_fedavg.json"
MODEL_FILE = "global_model_fedavg.pth"
...
def get_initial_parameters() -> fl.common.Parameters:
    ...
    print("✅ Server model fixed (BatchNorm → GroupNorm)")
...
def save_metrics_realtime(round_num, metrics):
    ...
    print(f"💾 [FedAvg] Metrics for Round {round_num} saved to {METRICS_FILE}")
...
def save_model_checkpoint(parameters, round_num):
    try:
        ...
        print(f"✅ [FedAvg] Model v{round_num} saved")
...
def aggregate_metrics_fn(metrics: List[Tuple[int, Dict]]) -> Dict:
    ...
    weighted_metrics = {
        ...
        "privacy_epsilon": 0.0, "privacy_delta": 0.0,
    }
    client_metrics = []
    privacy_risks = []
    ...
    print(f"\n{'='*70}")
    print(f"[FedAvg SERVER] Round Evaluation Report:")
    print(f"{'='*70}")
    ...
    print(f"   ► ε (epsilon):   {final_metrics['privacy_epsilon']:.4f}")
    print(f"{'='*70}\n")

    return {"accuracy": final_metrics['val_accuracy'], **final_metrics}


# --- FedAvg strategy subclass (NOT FedProx) ---
class FedAvgWithMetrics(fl.server.strategy.FedAvg):
    """Uses standard FedAvg instead of FedProx — no proximal term."""
...
if __name__ == "__main__":
    ...
    print("\n" + "="*70)
    print("🚀 [RUN 2] FedAvg BASELINE — Standard FedAvg WITH DP")
    print("="*70)
    print(f"Strategy:     FedAvg (NO proximal term)")
    print(f"DP:           ENABLED (ε=2.0, δ=1e-5, noise=1.1, clip=1.0)")
    ...
    # FedAvg — no proximal_mu parameter
    ...
    print("✅ Starting FedAvg server on 0.0.0.0:8081...")
    ...
    print("\n" + "="*70)
    print("🎉 [RUN 2] FedAvg TRAINING COMPLETE!")

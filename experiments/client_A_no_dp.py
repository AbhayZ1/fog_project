"""
RUN 1: No-DP Client A — NO Differential Privacy
...
print(f"\n{'='*70}")
print(f"🏥 [NO-DP] CLIENT A (Hospital A) - No Differential Privacy")
print(f"{'='*70}\n")
...
def train(net, trainloader, optimizer, epochs):
    ...
    for epoch in range(epochs):
        for i, (inputs, labels) in enumerate(trainloader):
            ...
            if i % 10 == 0:
                print(f"   [NO-DP Client A] Epoch {epoch+1}/{epochs}, Batch {i}/{len(trainloader)}, Loss: {loss.item():.4f}")
...
class PneumoniaClient(fl.client.NumPyClient):
    ...
    def fit(self, parameters, config):
        ...
        print(f"\n🔄 [NO-DP Client A] Starting local training (10 epochs)...")

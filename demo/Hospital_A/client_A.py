...
print(f"\n{'='*70}")
print(f"🏥 CLIENT A (Hospital A) - Initializing")
print(f"{'='*70}")
...
def load_data():
    ...
    # Data is now local to the client folder
    data_dir = os.path.join(os.path.dirname(__file__), "local_storage", "data")
...
def train(net, trainloader, optimizer, epochs: int):
    ...
    for epoch in range(epochs):
        for i, (inputs, labels) in enumerate(trainloader):
            ...
            if i % 10 == 0:
                print(f"   [Client A] Epoch {epoch+1}/{epochs}, Batch {i}/{len(trainloader)}, Loss: {loss.item():.4f}")
    
    # Calculate actual privacy spent
    epsilon = privacy_engine.get_epsilon(delta=TARGET_DELTA)
    print(f"   [Client A] Privacy spent: ε={epsilon:.2f} (target: {TARGET_EPSILON}, δ={TARGET_DELTA})")
...
class PneumoniaClient(fl.client.NumPyClient):
    ...
    def fit(self, parameters, config):
        ...
        print(f"\n🔄 [Client A] Starting local training (10 epochs)...")
    ...
    def evaluate(self, parameters, config):
        ...
        print(f"\n{'='*70}")
        print(f"[CLIENT A EVAL] Privacy Budget Status:")

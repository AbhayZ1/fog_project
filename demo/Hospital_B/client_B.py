...
print(f"\n{'='*70}")
print(f"🏥 CLIENT B (Hospital B) - Initializing")
print(f"{'='*70}")
...
def train(net, trainloader, optimizer, epochs: int):
    ...
    for epoch in range(epochs):
        for i, (inputs, labels) in enumerate(trainloader):
            ...
            if i % 10 == 0:
                print(f"   [Client B] Epoch {epoch+1}/{epochs}, Batch {i}/{len(trainloader)}, Loss: {loss.item():.4f}")
    
    # Calculate actual privacy spent
    epsilon = privacy_engine.get_epsilon(delta=TARGET_DELTA)
    print(f"   [Client B] Privacy spent: ε={epsilon:.2f} (target: {TARGET_EPSILON}, δ={TARGET_DELTA})")
...
class PneumoniaClient(fl.client.NumPyClient):
    ...
    def fit(self, parameters, config):
        ...
        print(f"\n🔄 [Client B] Starting local training (1 epochs)...")
    ...
    def evaluate(self, parameters, config):
        ...
        print(f"\n{'='*70}")
        print(f"[CLIENT B EVAL] Privacy Budget Status:")
